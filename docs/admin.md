...
**Drift** — the one-year cadence, the two dismissals, the quiet card marker and the "Been a while" panel — is Piece 4, and none of it appears yet. **Shared** (`person_works` / `person_fragments`) is Piece 3's tables. A domain that does not exist yet renders nothing at all, replaced by one plain sentence.
