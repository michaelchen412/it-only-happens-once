...
| **Song sheet** | slide-over, `/admin/listening` and `/admin/fragments` | **The one editor for a song** (§6b, [ADR 0031](adr/0031-a-song-carries-a-feeling-not-an-idea.md)). Its own paste bar, a player, and three tabs: **Feelings** (the default — the loop is paste, listen, press the words, Save), **Facts** (title/artist/album/year, Shared by, and the paired essay read-only), and **Notes** (one public, one private). The triad is the point — what it did to you, what is true about it, what you want to say — and it states ADR 0031's own claim in the interface: a feeling is not a property of the song, and a fact is. No subjects, no constellations, no "Why this one". **Publishes on save** (like quotes); unpublish via the list. |
...
3. **Songs auto-fetch from the link.** Paste a Spotify track/album or YouTube URL; the **Spotify Web API** fills title, artist, album and release year (keyless **oEmbed** is the fallback when no credential is configured), and the id + kind come from the URL. What no API can supply is **what the song did to you** — that is the feelings, pressed by hand while it plays, and nothing may propose them (§6b). See §6 for exactly what auto-fills and what stays manual.
...
- **A note anyone can open** → `fragments.body`. It renders in the music room behind a **†** at the end of a card's feelings line, and **nowhere else** — never above a player, which is the caption this site already removed once for saying what the embed says.
...
**The page is a list and a door; everything that edits a song is the SHEET** ([ADR 0031](adr/0031-a-song-carries-a-feeling-not-an-idea.md), 2026-08-11). It replaced a split that was never conceptual — **two editors divided by field**: this page owned the feelings and could not touch the metadata, while the Fragment Manager's sheet owned the metadata and knew nothing about feelings. Michael: *"this division is really uncomfortable for me, and I don't think it's a clean implementation."* The same sheet is mounted on `/admin/fragments`, so a song row there opens it too — the manager keeps **listing** songs (search, trash, bulk publish are corpus-wide) and stops **editing** them.

| The sheet | What it does |
|---|---|
| **Paste bar** | Inside the sheet, not on the page — a sheet with its own front door is mountable anywhere, which is what made one editor cheap. `songs.lookup` fills title/artist/album/year and reports whether the corpus already holds it; if it does, the whole row is pulled up instead, so pasting a link you already have is a way of **finding** it. Dedupe is on the **parsed** `{provider, kind, id}`, never the pasted string — the same track arrives as `?si=…`, as `intl-de/`, and as `spotify:track:…`, and a raw comparison grows a twin for each. |
| **The player** | Built server-side (`songEmbed`), because `lib/media.ts` reaches `astro:env/server` and nothing in `src/scripts/` may import it. ⚠ **Removing the iframe is the only pause there is** — a cross-origin frame cannot be controlled from outside, so closing the sheet, loading another song and saving all destroy it. Anything that *hides* a player instead leaves music playing in a room you have left. |
| **Feelings** *(the default tab)* | `feelings`, ordered dark → light by `sort`. Buttons with `aria-pressed`, not checkboxes: a row of words, not a form. First because the loop — paste, listen, press the words, Save — must never need a second tab. |
| **+ a word** | Creates a feeling mid-listen (§8) and presses it. Lands at the bright end until the Library places it. |
| **Facts** | Title, artist, album, the added year, **Shared by**, and the **paired essay read-only** — the most useful fact about 46 of the 48, and invisible in the workshop until now. Not editable here: pairing is the *essay's* decision (`songs.pair`, from its Music tab), and a second control would be a second write path. |
| **Notes** | One public, one private. See §6 above. |
| **Save** | One button, for the whole sheet. New song → `saveSong` (as **published**, not the `draft` default — see below) → `setFeelings` → `setNotes`. Existing song → `saveSong` **only if the metadata actually moved**, then the two relations. |
| **Two lists** | *Not yet heard* and *Filed*, on the page behind. A row moves between them on save, in place — no reload; neither list is a burn-down. |

**Why one Save rather than three immediate writes.** *"Applies immediately"* is the rule `pair` and the constellation picker follow, and it exists to stop a relation riding along with a **compare-and-set** save that could lose a rewrite. Both of those live on the writing sheet, which has such a save. **A song has none** — `saveSong` carries no `base_updated_at` and there is no autosave to collide with — so splitting the writes would buy nothing and would make *"have I saved?"* a question you answer by remembering which tab you were on. **Shared by** is the exception and keeps its own immediate contract, queueing its ticks until a new song has an id.

⚠ **`saveSong` runs only when the metadata changed**, and that is not a micro-optimisation: it re-derives the slug and upserts an author and a work every time, so a save that added one feeling would otherwise do three writes nobody asked for.
...
What a song does to you is a **feeling**, and a feeling is not a property of the song — *it is what happened in Michael*, which no model has access to. So there is no `suggestFeelings` beside this and there never will be (§6b). **No part of a song is machine-taggable**, which is a cleaner line than the one it replaced.
