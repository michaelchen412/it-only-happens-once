...
| **Song sheet** | slide-over, `/admin/listening` and `/admin/fragments` | **The one editor for a song** (§6b, [ADR 0031](adr/0031-a-song-carries-a-feeling-not-an-idea.md)). Its own paste bar, a player, and three tabs: **Feelings** (the default — the loop is paste, listen, press the words, Save), **The song** (title/artist/album/year, Shared by, and the paired essay read-only), and **Notes** (one public, one private). No subjects, no constellations, no "Why this one". **Publishes on save** (like quotes); unpublish via the list. |
...
| **The song** | Title, artist, album, the added year, **Shared by**, and the **paired essay read-only** — the most useful fact about 46 of the 48, and invisible in the workshop until now. Not editable here: pairing is the *essay's* decision (`songs.pair`, from its Music tab), and a second control would be a second write path. |
