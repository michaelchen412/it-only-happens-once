...
| **Notes** | `/admin/notes` | The pile (§5b). Every brain dump rendered as **its own text**, newest-touched first, with an elapsed stamp — no title, no slug, no checkbox, no table. Three controls per card: a pencil (or the words themselves) opens the note in **`NoteSheet`** — a drawer with the rest of the pile beside it — a bin deletes softly with an undo strip, and **→** opens the chooser holding the four destinations that CONSUME the note (task · log entry · quote · new piece · into an existing piece) and, under a rule, the **shelves** that keep it ([ADR 0042](adr/0042-a-shelf-is-where-a-jotting-lives.md)). Since 2026-09-01 the room opens on the **inbox** — the notes on no shelf — with a segment row of shelves beside a search field over the dump's own words; a live term escapes the inbox default and respects a pressed shelf (`search.md` §7b). ⚠ **The two controls answer to different rules** (2026-09-18): the search box waits for the pile to be past a glance, because a box over four jottings is furniture; the shelf row appears the moment a shelf exists, because the inbox HIDES a shelved note and the strip is the only door to the ones not on screen. One condition used to gate both, which made a shelf unreachable in a short pile — and no refresh helped, only a deeper pile. ⚠ **The vocabulary is groomed from the strip, not the Library** (2026-09-18): a pencil at the end of the shelf row opens a small manager — rename in place, delete (the confirm says how many notes go back to the inbox), and a create field that makes a word without filing anything. The → chooser also creates, and the two are not duplicates: the chooser makes a shelf **and files the note onto it** in one motion, which is a filing gesture that happens to need a new word; this one is the tidying gesture. ⚠ A rename **leaves the slug alone** — `?shelf=<slug>` addresses the filtered view and an unknown slug resolves to `activeShelf: null`, which would render the inbox as though nothing were wrong and break every bookmark silently. ⚠ There is **no merge**, deliberately: filing the notes across and deleting the empty shelf reaches the same place with every step visible.

⚠ **The drawer says where the open note lives** (2026-09-18, reported missing). `NoteSheet`'s head carries toggleable shelf chips beside the stamp and the word count — the card's own rule that *"where a thought LIVES is metadata sitting with the timestamp"*, applied to the surface whose stamp is in the head. ⚠ **Not a sixth row in the foot**: all five destinations down there CONSUME the note and a shelf is how a thought stays, which is why the → chooser already puts shelves below a separator. **The foot is where a note leaves; the head is where it lives.** ⚠ And **filing from the drawer does not eject the card** — in the pile the card going away IS the feedback, but in the drawer you are sitting reading the thing and the pile is the rail beside you.

⚠ **A shelf made from the → chooser now reaches the whole room**: both the menu row and the filter link are cloned from `<template>`s authored beside the loops that render the server's copies, so a glyph added to either lands in both. Before, the script hand-built a row with `textContent` — no tick element, and nothing added to the strip at all. Replaced `?view=notes` on 2026-08-03, which now redirects here. |
...
- **The icon badge is honest only while the app runs.** `navigator.setAppBadge` executes in the page, so an installed app keeps the numeral live while it is open and corrects it every time it is opened. ⚠ **It is cleared on `pagehide`, deliberately.** Leaving it standing is wrong every single night — at 00:01 the true count is 1 and the icon still shows yesterday's — and **being reliably absent beats being confidently wrong**. The cost is a blink on every admin navigation, taken knowingly. ⚠ Since 2026-09-18 a room is a **swap**, not a real page load ([ADR 0043](adr/0043-a-script-binds-per-arrival-not-per-document.md)) — `pagehide` no longer fires between rooms, so the blink is now only on leaving the Observatory altogether, which is strictly better than what this paragraph was written about.
...
## 3a. How you get between rooms

**The Observatory navigates client-side.** `<ClientRouter />` went on for `/admin`
on 2026-09-18, closing plan 24 · §9 — the public site had been on it for months
and the two halves now agree. A sidebar press swaps the body instead of
re-fetching, re-parsing and re-executing the whole admin bundle.

⚠ **The rule that comes with it is the important half**, and it is
[ADR 0043](adr/0043-a-script-binds-per-arrival-not-per-document.md): **a script
binds per arrival, not per document.** A module runs once per document and a swap
does not make a new one, so anything bound to a page element is bound to an
element that has been thrown away. `scripts/page.ts` is the one mechanism —
`onPage(boot)` re-runs the boot on every arrival, and the `page.on` it hands you
takes `document` and `window` listeners off again on the way out so they cannot
stack up.

⚠ **The first visit hides the whole class**, which is why this needed an
instrument rather than an audit. Astro executes scripts that are *new to the
page*, so walking into a room for the first time works perfectly and the second
visit is dead. `tests/e2e/walk.spec.ts` is the only spec in the suite that
navigates by **clicking** and arrives twice; the other 352 navigations are
`page.goto`, which is a full document load and would stay green with every
listener in the building dead. `src/tests/page-boot.test.ts` states the same rule
cheaply, and found four scripts a manual survey had missed.

⚠ **A component whose state should survive persists instead.** The ✚ carries
`transition:persist`, so its dialog — and the editor mounted inside it — travels
across the swap rather than being rebuilt. A half-typed thought now survives a
navigation, which is what that box exists to promise.

⚠ **Nothing in the sidebar prefetches.** `<ClientRouter />` turns prefetching on
by itself (`prefetchAll: true`, `hover`), and every admin response is
`Cache-Control: no-store` — which may not be stored, so the prefetch is discarded
and the real navigation fetches the page again. Hovering a row bought nothing and
cost a full SSR render plus its Supabase queries. `data-astro-prefetch="false"`
sits on the nav rows rather than `prefetchAll: false` in the config, because that
setting is site-wide and the public side genuinely benefits.

⚠ **`nav-progress.ts` still earns its place.** A swap is faster than a document
load and still fetches the next page, so the frame between the press and the
arrival is exactly as empty as it was.
...
- **⚠ The ✚ knows which shelf you are standing in (2026-09-18).** The `jot` tab
  carries a row of shelf chips, and **the shelf in `?shelf=` starts lit** — so
  pressing ✚ inside a shelf files the thought there without being asked, and a
  jot that belongs elsewhere is one tap from going there. This does not break
  "the ✚ belongs to the building": reading the URL is how it learns where you
  are standing *without* the layout having to tell it — no prop, no `locals`,
  and correct from a bookmark.

  ⚠ **An apparent selector, not a silent default** (Michael's call, asked
  directly). The chip is on screen, so the room's context is *offered* rather
  than imposed. The chips are `.lchip`, the toggle primitive the timeline
  already uses; a third pick replaces the oldest, which is the pile chooser's
  rule and the server's `MAX_SHELVES`. The vocabulary loads on first open, like
  the Log tab's roster and for the same reason.

  ⚠ **The row belongs to `jot` alone** — the other three destinations take the
  thought OUT of the pile, so a shelf on any of them would be filing something
  about to stop existing. And **the close now follows the note**: `landing()`
  relaxes the shelf filter, and only that, when the note did not end up on the
  shelf being viewed. It never tightens it — sending you to a shelf you did not
  press would be the ✚ navigating on your behalf.
...
**Library (`/admin/library`)** also carries **Export corpus** — the whole database as one JSON file, for the day you leave Postgres rather than for disaster recovery (which the nightly dump already covers). See [backups.md](backups.md). It grooms the three **cross-cutting** vocabularies: **subjects** (with definitions — the taxonomy the AI reads), **authors**, **works**. ⚠ **Shelves are deliberately NOT here.** They shipped in this room on 2026-09-18 and moved out the same day, to the pencil on the pile's filter strip (§5b). The test is [ADR 0040](adr/0040-a-view-state-is-not-a-room.md)'s: the other three appear on the manager, the composer, a person's page and the public site, so a room that holds all of them at once is the only place they can be groomed together — a shelf is private, notes-only, and exists in exactly one room. Michael: *"its not really intuitive to go into library."* Sending somebody here to rename a word that lives one room over spends the trip this room exists to save. Each row: edit in place, **merge** a duplicate into another (reassigns links, deletes the loser), or **delete** (FK-safe — `on delete set null`/cascade means a fragment is never orphaned). Usage counts show what's safe to remove.
...
**The field can make a real list, and only three fields in the building can** (2026-08-24). Notes takes bullet and numbered lists; a goal's *Why*, a foot above it in the same sheet, does not. That pair is the rule stated as a shape: a why is the reason a goal exists and reads as prose, notes are *how it is kept*, and a routine enumerates. The other two are a task's notes and an event's notes — the steps inside one thing, which is exactly the detail not worth splitting into more rows. **Everything else on the short-form editor stays bold-and-italic**: a quote is quoted words, a log entry answers *"what happened"*, a bio is what you'd say if someone asked, and a list there would change what the field is *for*. ⚠ **Headings, quotes and rules stay out of all fourteen**, and that is the line the tier holds — a heading turns text into a sectioned document, so a field that could carry one would be a page in disguise. **Storage did not change** and neither did the ruling above: `- [ ]` is still a plain bullet, which matters more now that the button makes the syntax easy to reach.
