...
| **The composer** | `/admin/constellations/[id]` | The composing room (design.md §13): suite rows in composed order (drag or nudge to reorder, ✕ unplaces), the constellation's name/slug/status/description/score, the three tests as quiet gauges, Preview → the real public page (drafts render for the admin only). One **Add** button opens the fragment browser. |
