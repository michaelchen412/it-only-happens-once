...
**Library (`/admin/library`)** also carries **Export corpus** — the whole database as one JSON file, for the day you leave Postgres rather than for disaster recovery (which the nightly dump already covers). See [backups.md](backups.md). It grooms the four cross-cutting vocabularies: **subjects** (with definitions — the taxonomy the AI reads), **authors**, **works**, and — since 2026-09-18 — **shelves**, the one vocabulary here that no reader ever sees ([ADR 0042](adr/0042-a-shelf-is-where-a-jotting-lives.md)). ⚠ **A shelf renames without touching its slug**, unlike the other three: `?shelf=<slug>` addresses the pile's filtered view, and an unknown slug resolves to `activeShelf: null` — so a re-derived slug would render the inbox as though nothing were wrong, breaking every bookmark silently. The row prints the slug read-only so the control cannot look like it renames the URL too. ⚠ **And a shelf has no merge**, deliberately: filing the notes across and deleting the empty shelf reaches the same place with every step visible, so `VocabActions` takes `mergeable={false}` rather than offering a control that could only fail. Deleting one takes nothing — `fragment_shelves` cascades, so the jottings simply go back to the inbox, and the confirm says so instead of borrowing the other three's wording. Each row: edit in place, **merge** a duplicate into another (reassigns links, deletes the loser), or **delete** (FK-safe — `on delete set null`/cascade means a fragment is never orphaned). Usage counts show what's safe to remove.
