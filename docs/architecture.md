...
- **⚠ A public page whose READ FAILED flips to `private, no-store` too** (2026-08-27). A failed query degrades to an empty state rather than a 500 — right for the reader, and dangerous for the CDN: `s-maxage` pins that empty page at the edge and `stale-while-revalidate` then serves it for a day behind the revalidation, so one transient error becomes an outage every stranger sees. Every public list route branches on it — `/`, `/blog`, `/about`, `/listening`, `/rss.xml`, `/sitemap.xml`. What makes the branch possible is that `listConstellations` answers `null` for *failed* and `[]` for *nothing to show*; they were the same value until a single 401 emptied the front door.
- **⚠ A PERMALINK whose read failed answers 5xx, not 404.** `/blog/[slug]` and `/{slug}` have no empty state to degrade into: the only "empty" they can render is *this does not exist*, which a crawler is entitled to believe about a published essay. `required()` in [`lib/read-log.ts`](../src/lib/read-log.ts) throws for those reads instead, landing on `500.astro` — `no-store`, a status that means try again, and the truth. A **miss** is untouched: an unknown slug still 404s, and an unknown constellation still drifts home.
