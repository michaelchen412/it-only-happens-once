...
onPage((page) => {
  ...
  if (zone) {
    ...
    if (!writable) {
      ...
      /*
        ⚠ TORN DOWN ON THE WAY OUT (plan 24 · §9). `onPage` re-runs this boot on
        every client-side arrival, so walking back onto Today mounts a second
        pair of editors into the new elements while the first pair stays alive,
        reachable through its own ProseMirror listeners and never collected.
      */
      page.cleanup(() => {
        for (const handle of proseFields.values()) handle.editor.destroy();
        proseFields.clear();
      });
