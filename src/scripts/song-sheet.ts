...
/*
  ⚠ EVERY ARRIVAL (plan 24 · §9) — and the two functions below stopped being
  `export`ed with the same change. Nothing outside this file ever imported them:
  the pages that mount this sheet reach it through `song:edit` on `document`,
  which is what the comment above `openSongSheet` already says ("the pages that
  mount this sheet are the only callers"). Keeping them exported would have
  meant keeping them at module scope, closing over a `sheet` element that the
  router throws away on the first navigation — an exported handle to a dead
  dialog, which is worse than no handle at all.
*/
onPage((page) => {
   ...
   */
  function openSongSheet(seed: Partial<SongSheetSeed> = {}): void {
  ...
  async function openSongById(id: string): Promise<void> {
