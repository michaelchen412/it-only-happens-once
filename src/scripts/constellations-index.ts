...
onPage(() => {
  ...
  */
  const newForm = document.getElementById('new-constellation') as HTMLFormElement | null;
  const newName = document.getElementById('cl-new-name') as HTMLInputElement | null;
  ...
  function openRowMenu(trigger: HTMLButtonElement, row: HTMLElement) {
    ...
    statusRow.title = published
      ? 'Take it out of the public sky — it stays here as a draft'
      : 'Put it in the public sky';
