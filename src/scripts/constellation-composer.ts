...
onPage((page) => {
  /*
    ⚠ A NULL CHECK, NOT `!`. The assertion was safe while this module only ran
    on the page that has `#ccomposer`; under the router its boot fires on every
    arrival anywhere in the building, so `!` would throw on the first room that
    is not the composer — and a throw inside a boot takes the rest of that
    arrival's bindings with it.
  */
